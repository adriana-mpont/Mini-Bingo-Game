...
class BingoCard:
    ...
    def validate_card(self) -> bool:
        ...
        #Checking that all the numbers in the card are unique (no diplicates).
        #And checking that every numebr is withing the allowed range. 
        return len(flat_numbers) == len(set(flat_numbers)) and all(
            1 <= n <= self.number_range for n in flat_numbers
        )

    #Defyning a helper method that returns all numbers in a single flat list.
    ...
    def check_line(self) -> bool:
        ...
        #Returning False is no new row or columns has been comleted. 
        return False
