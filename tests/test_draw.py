...
#Defining a test function to ensure that the drawn numbers are within the valid limits. 
def test_draw_returns_number_in_range():
    ...
    #Failing the test if the number falls outside of the range 1 to 99.
...
def test_draw_no_repetition():
    #Creating a new number drawer instance.
    drawer = NumberDrawer()
    #Creating a set to track the numbers already drawn. 
    drawn_numbers = set()
    #Creating a loop through 99 draws (which is the maximum possible). 
    for _ in range(99):
        #Drawing one random number.
        num = drawer.draw_number()
...
def test_display_drawn_numbers(capsys):
    ...
    #Failing the test is the output does not appear as expected (with the title at the top).
